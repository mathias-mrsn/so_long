...
void __put_img_to_win__(t_data *s, int a, int b, t_mlx img)
{
	...
	while (i < img.height)
	{
		...
		while (j < img.width)
		{
			__pixel_on_img__(s->mlx, a + i, b + j, color[(int)j * img.height + (int)i]);
			j += 1.0;
		}
		i += 1.0;
	}
...
{
	int *color;
	const double ratio_x = s->mlx->width / s->map->width;
	const double ratio_y = (s->mlx->height - STATUS_BAR) / s->map->height;
	double i;
	double j;
	...
	while (i < ratio_x)
	{
		...
		while (j < ratio_y)
		{
			__pixel_on_img__(s->mlx, b * ratio_x + i, (a * ratio_y) + j + STATUS_BAR,
...
t_mlx __get_img__(t_data *s, char *pathfile)
{
	...
	img.img = mlx_xpm_file_to_image(s->mlx->mlx, pathfile, &img.height, &img.width);
	img.addr = mlx_get_data_addr(s->mlx->mlx, &img.bits_per_pixel, &img.size_line, &img.endian);
	return (img);
}