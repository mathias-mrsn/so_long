...
t_ennemies	*ft_create_elem(int a, int b)
{
	t_ennemies *new;
...
void		ft_create_ennemies(int i, int j)
{
	t_ennemies *new;

	if (!x()->ennemies)
		x()->ennemies = ft_create_elem(i, j);
	else
	{
		new = ft_create_elem(i, j);
		new->next = x()->ennemies;
		x()->ennemies = new;
	}
}
...
void	ft_move_ennemie_up_right(int *nbr)
{
	(*nbr)++;
	ft_ennemies_on_player();
}

void	ft_move_ennemie_down_left(int *nbr)
{
	(*nbr)--;
	ft_ennemies_on_player();
}
...
{
	t_ennemies	*tmp;
	const	rand = ft_rand(4);

	printf("%d", rand);
	tmp = x()->ennemies;
	while(tmp)
	{
		if(ft_rand(ENNEMIES_SPEED) == 0)
		{
			if(rand == 0 && x()->map->map[tmp->y - 1][tmp->x] != '1' && !ft_is_ennemies(tmp->y - 1, tmp->x))
				ft_move_ennemie_down_left(&tmp->y);
			else if(rand == 1 && x()->map->map[tmp->y + 1][tmp->x] != '1' && !ft_is_ennemies(tmp->y + 1, tmp->x))
				ft_move_ennemie_up_right(&tmp->y);
			else if(rand == 2 && x()->map->map[tmp->y][tmp->x - 1] != '1' && !ft_is_ennemies(tmp->y, tmp->x - 1))
				ft_move_ennemie_down_left(&tmp->x);
			else if(rand == 3 && x()->map->map[tmp->y][tmp->x + 1] != '1' && !ft_is_ennemies(tmp->y, tmp->x + 1))
				ft_move_ennemie_up_right(&tmp->x);
		}
		tmp = tmp->next;
	}
...
int	ft_put_texture(int	a, int b, t_mlx *mlx, t_mlx img)
{
	int 	*color;
	...
	while(i < ratio_x)
	{
		...
		while(j < ratio_y)
		{
			put_pixel_on_img(x()->mlx, b * ratio_x + i , (a * ratio_y) + j + 70, color[(int)(j / (ratio_y / img.height))  * img.height + (int)(i / (ratio_x / img.width))]);
	...
	while(x()->map->map[i])
	{
		...
		while(x()->map->map[i][j])
		{
			if(i == x()->map->player_x && j == x()->map->player_y)
				ft_put_texture(i, j, mlx, ft_get_img(PLAYER)); // i == y and j == x
			else if (ft_is_ennemies(j, i))
				ft_put_texture(i, j, mlx, ft_get_img(ENNEMIE));
			else
...
{
	const struct s_texture	textfile[5] = {{'1', WALL},{'0', GRASS},{'C', COLLECTIBLE_1},{'E', EXIT},{'P', PLAYER}};
...
int	main(int argc, char **argv)
{
	...
	mlx_hook(x()->mlx->win, 3, 1L<<0, NULL, NULL);
